...
headers = {
    ...
    'Referer': 'https://web.vip.miui.com/page/packet2017?showLatest=true&userId=8790860',
...
function grabPacket(pid, sid) {
    ...
    request(
        {url: 'https://web.vip.miui.com/api/packet/grab?pid=' + pid + '&sid=' + sid, headers: headers},
        ...
        function (error, response, body) {
            if (!error && response.statusCode == 200) {
                ...
                for (var i = 0; i < info.packets.length; i++) {
                    ...
                    console.log("[Packet] ID: " + info.packets[i].packetId + " - Name: " + info.packets[i].name + ' - Level: ' + info.packets[i].level + ' - Remain: ' + remainTime + " ms" + ((remainTime < interval) ? " [Grab]" : ""));
